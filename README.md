...
- After Freysa finishes speaking the fifth sponsored ElevenLabs response, the composer is replaced by a one-click Freysa-on-X prompt. Send remains disabled during speech so a sixth request cannot bypass that handoff. A successful unlock restores the composer with five additional responses for the day. After all ten are used, the composer offers a personal ElevenLabs key or Microsoft TTS instead.
