...
- ElevenLabs character timing is coalesced, reduced, and temporally smoothed before it reaches the avatar so letter-by-letter timing does not create oversized, rapid mouth movement. Azure's native facial frames remain unchanged.
- The Settings panel puts facial-performance controls first, then voice provider and voice selection, provider-safe speaking speed, expandable pronunciation rules, sponsored usage, and a curated 12-voice ElevenLabs shortlist led by Matilda.
- After Freysa finishes speaking the fifth sponsored ElevenLabs response, the composer is replaced by a one-click Freysa-on-X prompt. Send remains disabled during speech so a sixth request cannot bypass that handoff. A successful unlock restores the composer with five additional responses for the day. After all ten are used, the composer stays blocked until Microsoft TTS is selected or a personal ElevenLabs key is verified inline.
